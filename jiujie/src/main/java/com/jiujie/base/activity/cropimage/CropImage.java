...
{
//	public static final File FILE_SDCARD = new File(Environment.getExternalStorageDirectory()+"/CaiBa/picture/");
	public static File FILE_SDCARD;
...
//	}
	 ...
	 * 
	 * @param width  截图后的图片宽
	 * @param height 截图后的图片高
	 * @param savePath 存储截图路径,不要加格式
	 */
	public CropImage(Context context, CropImageView imageView,Handler handler,int width,int height,String savePath,String saveName)
	{
		mContext = context;
		mImageView = imageView;
		mImageView.setCropImage(this);
		...
		mWidth = width;
		mHeight = height;
		FILE_SDCARD = new File(savePath);
		FILE_LOCAL = new File(FILE_SDCARD,saveName);
	}
...
//                mRunFaceDetection.run();
    ...
    private Bitmap onSaveClicked(Bitmap bm) {
        // CR: TODO!
        // TODO this code needs to onCheckedChanged to use the decode/crop/encode single
...
//        int height = 70;//dr.height();
        int width = mWidth;
        int height = mHeight;//设置剪切后的宽高,我需要60dp的图,so...120,为清晰些,搞150好了
        if(width<=0||height<=0){
        	width = 150;
        	height = 150;
        }
...
//    	}
    	if(FILE_LOCAL==null){
    		FILE_LOCAL = new File(FILE_SDCARD, "defaultPic");
    	}
    	if(!FILE_LOCAL.exists()){
    		FILE_LOCAL.mkdirs();
    	}
    	String path = FILE_LOCAL+"mm.jpg";
//    	String path = FILE_LOCAL+"mm.png";
    	try
		{
    ...
    Runnable mRunFaceDetection = new Runnable() {
        ...
        private Bitmap prepareBitmap() {
            ...
            if (mBitmap.getWidth() > 256) {
                mScale = 256.0F / mBitmap.getWidth(); // CR: F => f (or onCheckedChanged
                                                      // all f to F).
            }
