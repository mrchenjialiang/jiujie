...
public class VideoViewUtil implements VideoController{
    private final String TAG = "VideoViewUtil";
    ...
    public VideoViewUtil(MyVideoView mVideoView, boolean isLoop, boolean isAutoStart, ViewGroup containerView) {
        ...
        if(Build.VERSION.SDK_INT>=17){
            mVideoView.setOnInfoListener(new MediaPlayer.OnInfoListener() {
                ...
                public boolean onInfo(MediaPlayer mp, int what, int extra) {
                    switch (what) {
                        case MediaPlayer.MEDIA_INFO_BUFFERING_START:
                            UIHelper.showLog(VideoViewUtil.this,"VideoView.setOnInfoListener buffering true");
                            if(onVideoStatusListener !=null){
                                onVideoStatusListener.onBufferListen(true);
                            }
                            break;
                        case MediaPlayer.MEDIA_INFO_BUFFERING_END:
                            UIHelper.showLog(VideoViewUtil.this,"VideoView.setOnInfoListener buffering false");
                            if(onVideoStatusListener !=null){
                                onVideoStatusListener.onBufferListen(false);
                            }
                            break;
